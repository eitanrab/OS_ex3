#include "resources/MapReduceFramework.h"
#include <cstdio>
#include <array>
#include <unistd.h>
#include <fstream>
        ...
        while (state.stage != REDUCE_STAGE || state.percentage != 100.0)
        {
            if (last_state.stage != state.stage || last_state.percentage != state.percentage) {
                ...
                if (last_state.stage == state.stage && state.percentage < last_state.percentage) {
                    FAIL() << "Bad percentage(smaller than previous percentage at same stage): " << state.percentage << "(" << last_state.percentage << ")" << ", encountered during stage " << state.stage << ")";
                }
